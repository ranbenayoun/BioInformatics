...
(1) whole blood transcriptome is a record of the genes which were expressed into protein in the blood cells (white blood cells) using RNA sequencing.
(2) The researchers wanted to determine the effect of DHA (add two-three words on what is DHA in brackets) and blueberries after a high fat meal. They measured the RNA expressed during digestion of each compound in regards to the time of day and fasting hours.
```
...
(1) PCA reduces the number of dimentions by projecting the data on less dimentions (2). This is done by lineariliy transforming the data to a new coordinate system that allows easy identification of the directions that capture the largest variation in the data. This way we can categories the data into clusters and visualize it easier.
...
1. No, from the PCA we could deduce the data can be clustered into 4 clusters, whereas the clustering result shows 3 clusters. Also, the data distribution is different. <br/>
2. Each indevidual base-level of gene expression is different, so it's a variation the researchers couldn't isolate between subjects. we can see that clearly in the PCA plot with each cluster corresponds to a different subject. <br/> (You write in 1 that there are 4 clusters, but here you say that "each cluster corresponds to a different subject" - where there seem to be 5 subjects in the plot, not 4)
3. To achieve a well comprehensive research, we need to test different subjects to be able to draw conclutions on the general population, so yes we have to have various subjects with their gene expression. <br/>
...
(If I understood correctly, the addition of the day to the grouping improved the clustering. So the question is why this change of adding the day is expected to improve the results?)

My version:
The new object differs from the original by a significant increase in variance explained by PC1 (98.9% vs. 31.5%). One major change in the new design is the addition of “Day” to the PCA grouping beside “Subject.” By adding “Day” to the design, we consider the environmental changes (e.g., temperature, humidity, light exposure, or lab conditions) and human factors (e.g., different technicians handling samples, variation in pipetting accuracy, instrument calibration, reagent quality, or time of sample processing) that can change between days. Considering these changes ensures PC1 captures the biological signal, making more accurate results.
```
...
padj - the adjusted p-value using the Benjamini-Hochberg procedure to control the false discovery rate (FDR). 
...
```{}
Write your answer here:
1.	RRM2 (Ribonucleotide Reductase Regulatory Subunit M2):
  This gene encodes one of the two subunits of ribonucleotide reductase, an enzyme responsible for   catalyzing the formation of deoxyribonucleotides from ribonucleotides. The protein (M2) is         synthesized in a cell-cycle-dependent manner, and transcription from this gene can result in       two isoforms due to alternative promoter usage.
2.	HIST1H3B (Histone Cluster 1, H3b):
  HIST1H3B encodes a replication-dependent histone that is part of the histone H3 family. Histones   are nuclear proteins that form the core of nucleosomes, around which DNA is wrapped. This          nucleosome structure is essential for chromatin organization and higher-order chromatin            compaction. The gene is intronless, located in the histone gene cluster on chromosome 6p22-p21.3,   and its transcripts terminate with a palindromic element rather than a polyA tail.
3.	IGLL5 (Immunoglobulin Lambda-Like Polypeptide 5):
  IGLL5 encodes one of the immunoglobulin lambda-like polypeptides. Unlike other immunoglobulin      genes, it does not require somatic rearrangement for expression. Its first exon is unrelated to    immunoglobulin variable genes, while its second and third exons correspond to the immunoglobulin   lambda joining 1 and lambda constant 1 gene segments. Alternative splicing generates multiple      transcript variants.

Summary of Shared Characteristics:
  RRM2, HIST1H3B, and IGLL5 are involved in essential cellular processes such as DNA synthesis,      chromatin organization, and immune system function. These genes exhibit tightly regulated          expression patterns and structural specializations that are critical for their respective roles    in maintaining cellular and physiological homeostasis.

Reference: www.genecards.org
```
